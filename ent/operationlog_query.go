...
func (olq *OperationLogQuery) QueryOperator() *UserQuery {
	...
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		...
		step := sqlgraph.NewStep(
			...
			sqlgraph.Edge(sqlgraph.M2O, true, operationlog.OperatorTable, operationlog.OperatorColumn),
		)
...
func (olq *OperationLogQuery) Clone() *OperationLogQuery {
	...
	return &OperationLogQuery{
		...
		withOperator: olq.withOperator.Clone(),
...
func (olq *OperationLogQuery) WithOperator(opts ...func(*UserQuery)) *OperationLogQuery {
	query := (&UserClient{config: olq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	olq.withOperator = query
	return olq
}
...
//		GroupBy(operationlog.FieldName).
//		Aggregate(ent.Count()).
//		Scan(ctx, &v)
...
func (olq *OperationLogQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*OperationLog, error) {
	var (
		...
		_spec       = olq.querySpec()
		loadedTypes = [1]bool{
			olq.withOperator != nil,
		}
	)
	if olq.withOperator != nil {
		withFKs = true
	}
	if withFKs {
		_spec.Node.Columns = append(_spec.Node.Columns, operationlog.ForeignKeys...)
	}
	...
	if query := olq.withOperator; query != nil {
		if err := olq.loadOperator(ctx, query, nodes, nil,
			func(n *OperationLog, e *User) { n.Edges.Operator = e }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (olq *OperationLogQuery) loadOperator(ctx context.Context, query *UserQuery, nodes []*OperationLog, init func(*OperationLog), assign func(*OperationLog, *User)) error {
	ids := make([]int, 0, len(nodes))
	nodeids := make(map[int][]*OperationLog)
	for i := range nodes {
		if nodes[i].user_operating_record == nil {
			continue
		}
		fk := *nodes[i].user_operating_record
	...
	for _, n := range neighbors {
		...
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "user_operating_record" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
